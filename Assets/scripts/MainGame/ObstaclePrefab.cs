    ...
    void Start()
    {
        ...
        for (int i = 0; i < ObstaclePoolSize; i++)
        {
            if (i == random_check - 1)
            {
                ...
                Obstacles[i] = (GameObject)Instantiate(Obstacle2, obstacle_Position, Quaternion.identity, Parent.transform);
            }
            else
            {
                Obstacles[i] = (GameObject)Instantiate(Obstacle, obstacle_Position, Quaternion.identity,Parent.transform);
            }
        }
    }
